...
{ 
  ...
  function gf_get_latest_book($sISBN=null) {
    $sql = "call sp_query('select a.*, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/thumbnail/'', a.sFileName) as sNewPathCoverThumbnail, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/original/'', a.sFileName) as sNewPathCoverOriginal, b.nCount as nViews from tm_pisc_book a left join tx_pisc_view_count b on b.sISBN = a.sISBN where a.sPathCover is not null and a.sISBN is not null and a.sFileName is not null and trim(a.sFileName) <> '''' and trim(a.sISBN) <> '''' and a.sISBN <> ''-'' ";
		if($sISBN !== null) {
			$sql .= " and a.sISBN = ''".trim($sISBN)."''";
			//-- Insert view
		}
		$sql .= " ORDER BY RAND() limit ".(($sISBN !== null) ? 1 : 12)."');";
		if($sISBN !== null) {
	...
	function gf_search($sParams=null) {
		...
		$sql .= "select a.*, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/original/'', a.sFileName) as sNewPathCoverOriginal, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/thumbnail/'', a.sFileName) as sNewPathCoverThumbnail, b.nCount as nViews from tm_pisc_book a left join tx_pisc_view_count b on b.sISBN = a.sISBN where trim(a.sJudulPerubahan) like ''%".str_replace("'", "`", urldecode(trim($sParams)))."%'' and (a.sISBN is not null and trim(a.sISBN) <> '''' and trim(a.sISBN) <> ''-'') ";
		$sql .= " union all ";
		$sql .= "select a.*, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/original/'', a.sFileName) as sNewPathCoverOriginal, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/thumbnail/'', a.sFileName) as sNewPathCoverThumbnail, b.nCount as nViews from tm_pisc_book a left join tx_pisc_view_count b on b.sISBN = a.sISBN where trim(a.sJudulRC) like ''%".str_replace("'", "`", urldecode(trim($sParams)))."%'' and (a.sISBN is not null and trim(a.sISBN) <> '''' and trim(a.sISBN) <> ''-'') ";
		$sql .= " union all ";
		$sql .= "select a.*, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/original/'', a.sFileName) as sNewPathCoverOriginal, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/thumbnail/'', a.sFileName) as sNewPathCoverThumbnail, b.nCount as nViews from tm_pisc_book a left join tx_pisc_view_count b on b.sISBN = a.sISBN where (trim(a.sDetailPengarang) like ''%".str_replace("'", "`", urldecode(trim($sParams)))."%'' or trim(a.sNamaPengarangCover) like ''%".str_replace("'", "`", urldecode(trim($sParams)))."%'') and (a.sISBN is not null and trim(a.sISBN) <> '''' and trim(a.sISBN) <> ''-'') ";
		$sql .= " union all ";
		$sql .= "select a.*, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/original/'', a.sFileName) as sNewPathCoverOriginal, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/thumbnail/'', a.sFileName) as sNewPathCoverThumbnail, b.nCount as nViews from tm_pisc_book a left join tx_pisc_view_count b on b.sISBN = a.sISBN where trim(a.sNoProduk) = ''".str_replace("'", "`", urldecode(trim($sParams)))."'' and (a.sISBN is not null and trim(a.sISBN) <> '''' and trim(a.sISBN) <> ''-'') ";
		$sql .= " union all ";
