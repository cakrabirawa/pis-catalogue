...
{ 
  ...
  function gf_get_latest_book($sISBN=null) {
    $sql = "call sp_query('select a.*, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/thumbnail/'', a.sFileName) as sNewPathCoverThumbnail, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/original/'', a.sFileName) as sNewPathCoverOriginal, (select sum(p.nViews) as nViews from tx_pisc_view p where p.sISBN = a.sISBN) as nViews from tm_pisc_book a where a.sPathCover is not null ";
		if($sISBN !== null) {
	...
	function gf_search($sParams=null) {
		$sql = "call sp_query('select a.*, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/original/'', a.sFileName) as sNewPathCoverOriginal, concat(''".$this->sPathCover."cover'', ''/'', a.sIdNaskah, ''/thumbnail/'', a.sFileName) as sNewPathCoverThumbnail, (select sum(p.nViews) as nViews from tx_pisc_view p where p.sISBN = a.sISBN) as nViews from tm_pisc_book a where a.sJudulPerubahan like ''%".str_replace("'", "`", urldecode(trim($sParams)))."%'' and (a.sISBN is not null and trim(a.sISBN) <> '''' and trim(a.sISBN) <> ''-'') ');";
		//--------------------------------------------------------
