...
<div class="nav-tabs-custom">
	<ul class="nav nav-tabs">
		<li class="active"><a data-toggle="tab" href="#tab_1">Form Auth Open Reports</a></li>
	</ul>
	<div class="tab-content">
		<form id="frmCoreUserAuth" name="frmCoreUserAuth" action="<?php print site_url(); ?>c_core_open_reports_auth/gf_transact/" method="post">
...
<script>
	var sSURL = "<?php print site_url(); ?>c_core_open_reports_auth/gf_upload/";
	var sParam = "";
	var dialog = "";
		...
		$("button[id='button-submit']").click(function() {
			if ($.trim($(this).html()) === "Cancel")
				...
				oObjDivResult.find("table tr:gt(0)").each(function(i, n) {
					var nIdOpenReport = $(this).find("td:eq(0)").html();
					...
					objForm.append("<input type=\"hidden\" name=\"nIdOpenReport[]\" id=\"nIdOpenReport\" value=\"" + nIdOpenReport + "\" />");
	...
	function gf_load_data_auth() {
		$.ajax({
			type: "POST",
			url: "<?php print site_url(); ?>c_core_open_reports_auth/gf_load_auth_menu/",
			data: {
	...
	function gf_bind_click() {
		$("#div-auth-content").find("table tr:gt(0)").find("td:eq(3)").addClass("text-center");
